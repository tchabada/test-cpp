...
WatchdogManager::check()
{
    ...
    return std::ranges::all_of(watchdogs_, [timePoint = IWatchdog::Clock::now()](const auto& watchdog) -> bool {
...
WatchdogManager::registerWatchdog(const std::shared_ptr<IWatchdog>& watchdog)
{
    ...
    if (std::ranges::find(watchdogs_, watchdog) == watchdogs_.end()) {
