...
TEST(FsmTest, testTransition)
{
    ...
    EXPECT_CALL(machine.state<Stopped>(), react(An<const UpdateEvent&>())).Times(1);
    EXPECT_CALL(machine.state<Stopped>(), react(An<const ToggleEvent&>()))
        .WillOnce([&machine](const ToggleEvent&) { machine.state<Stopped>().transit<Started>(); });

    EXPECT_CALL(machine.state<Started>(), entry()).Times(1);
    EXPECT_CALL(machine.state<Started>(), exit()).Times(1);
    EXPECT_CALL(machine.state<Started>(), react(An<const UpdateEvent&>())).Times(1);
    EXPECT_CALL(machine.state<Started>(), react(An<const ToggleEvent&>()))
        .WillOnce([&machine](const ToggleEvent&) { machine.state<Started>().transit<Stopped>(); });
