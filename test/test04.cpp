...
#include "example04/mixins.hpp"

using namespace testing;

template <typename Base> struct NormalMixin : Base
{
...
template <typename Base> struct MixinWithParameter : Base
{
    template <typename... Args>
    MixinWithParameter(int x, Args&&... rest)
...
template <typename T, typename Base> struct TemplatedMixinWithParameter : Base
{
    template <typename... Args>
    TemplatedMixinWithParameter(const T& x, Args&&... rest)
        : Base(std::forward<Args>(rest)...)
        , x_{x}
    {
    }

    const T& get() const { return x_; };
...
template <typename Base> struct Sender : Base
{
...
template <typename Base> struct Receiver : Base
{
...
template <typename Base> struct Updater : Base
{
...
template <typename Base> struct IsSocket : Base, AbstractSocket
{
...
{
public:
    void SetUp() override { ::testing::Test::RecordProperty("sw_unit_foreign_id", "ptmm::mixins::Mixins"); }
};
